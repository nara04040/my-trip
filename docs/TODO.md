...
- [x] `lib/types/` 디렉토리
  ...
  - [x] `tour.ts` 반려동물 타입 추가 (MVP 2.5)
    - [x] PetTourInfo 인터페이스 정의
    - [x] chkpetleash (반려동물 동반 여부)
    - [x] chkpetsize (반려동물 크기)
    - [x] chkpetplace (입장 가능 장소)  
    - [x] chkpetfee (추가 요금)
    - [x] petinfo (기타 반려동물 정보)
    - [x] parking (주차장 정보)
    - [x] PetSize 타입 및 PET_SIZE_NAMES 상수 정의
    - [x] PetPlace 타입 및 PET_PLACE_NAMES 상수 정의
- [x] `lib/api/` 디렉토리
...
- [x] 환경변수 설정
  ...
  - [x] `.env` 파일에 네이버 지도 클라이언트 ID 설정 (NEXT_PUBLIC_NAVER_MAP_CLIENT_ID)
...
- [x] `components/tour-filters.tsx` 필터 컴포넌트
  ...
  - [x] 반려동물 동반 가능 필터 추가 (MVP 2.5)
    - [x] 반려동물 필터 토글 UI
    - [ ] 반려동물 크기별 필터 (소형, 중형, 대형) - 향후 구현
    - [ ] 실내/실외 동반 가능 여부 필터 - 향후 구현
    - [x] 필터 상태 관리 (URL searchParams에 pet 파라미터 추가)
    - [x] API 연동 시 반려동물 관련 파라미터 전달 (detailPetTour2 API 병렬 호출)
...
- [x] `components/naver-map.tsx` 네이버 지도 컴포넌트
  - [x] 기본 지도 표시 (Naver Maps API v3 NCP)
  ...
  - [x] 마커 클릭 시 인포윈도우
    ...
    - [x] 간단한 설명 (주소)
    - [x] "상세보기" 버튼
  - [x] 마커 색상 구분 (관광 타입별, 선택 사항)
    - [x] 관광 타입별 색상 매핑 정의
    - [x] 마커 아이콘 생성 함수 구현
    - [x] 마커 생성 시 색상 적용
    - [x] 지도 범례 컴포넌트 추가
  - [x] 지도 컨트롤 (줌 인/아웃, 지도 유형 선택 - 네이버 지도 기본 컨트롤 사용)
- [x] 리스트-지도 연동
  - [x] 리스트 항목 클릭 시 해당 마커로 지도 이동
  - [x] 리스트 항목 호버 시 해당 마커 강조 (선택 사항)
    - [x] 마커 강조 아이콘 생성 함수 추가
    - [x] NaverMapRef 인터페이스 확장 (highlightMarker, unhighlightMarker)
    - [x] 마커 강조/해제 메서드 구현
    - [x] TourCard 호버 이벤트 처리 (데스크톱만 동작)
- [x] 반응형 레이아웃
  - [x] 데스크톱: 리스트(좌측) + 지도(우측) 분할 레이아웃
  - [x] 모바일: 탭 형태로 리스트/지도 전환
  - [x] 지도 최소 높이 설정 (400px 모바일, 600px 데스크톱)
...
- [x] `components/tour-detail/detail-pet-tour.tsx` 반려동물 정보 컴포넌트
  - [x] `detailPetTour2` API 연동
  - [x] 반려동물 동반 가능 여부 표시
    - [x] 반려동물 아이콘 (🐾) 표시
    - [x] 동반 가능/불가능 상태 명확히 표시
  - [x] 반려동물 크기 제한 정보
    - [x] 소형견 가능 여부
    - [x] 중형견 가능 여부
    - [x] 대형견 가능 여부
  ...
  - [x] 접근성
    ...
    - [x] 키보드 네비게이션
- [x] 상세페이지에 반려동물 섹션 통합
  - [x] `app/places/[contentId]/page.tsx`에 DetailPetTour 컴포넌트 추가
  - [x] 반려동물 정보 섹션 배치 (기본 정보 다음)
  - [x] 반려동물 정보가 없으면 섹션 숨김
  - [x] 로딩 상태 처리 (Server Component이므로 불필요)
  - [x] 에러 처리
...
- [x] `app/stats/page.tsx` 통계 대시보드 페이지 생성
...
- [x] 네비게이션에 통계 페이지 링크 추가
  - [x] `components/navbar.tsx` 또는 헤더에 "통계" 메뉴 추가
  - [x] 아이콘 추가 (BarChart3 또는 PieChart)
- [x] 메타데이터 설정
  - [x] 페이지 제목 설정 (예: "통계 - My Trip")
  - [x] 페이지 설명 설정
